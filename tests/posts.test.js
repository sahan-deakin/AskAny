...
describe('Posts Routes', () => {
    ...
    it('GET /api/posts - returns empty array initially', async () => {
        const res = await request(app).get('/api/posts')
        expect(res.statusCode).toBe(200)
        expect(Array.isArray(res.body)).toBe(true)
    })

    it('POST /api/posts - creates a post when logged in', async () => {
        const res = await request(app)
            .post('/api/posts')
            .set('Authorization', `Bearer ${token}`)
            .send({ title: 'How do I use Docker?', body: 'Looking for resources' })
    ...
    it('DELETE /api/posts/:id - non-owner cannot delete', async () => {
        const create = await request(app)
            .post('/api/posts')
            .set('Authorization', `Bearer ${token}`)
            .send({ title: 'Protected', body: 'Body' })
